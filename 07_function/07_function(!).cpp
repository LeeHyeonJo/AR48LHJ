...
// **함수 오버로딩
// 함수의 이름을 같게 써도, 전달인자를 다르게 하면 다른 함수로 취급하는 문법
int plus(int a)
{
}
char plus(char n)
...
// ㄴ 메인에서 전달해줄때는 전달하는 변수의 자료형에 따라 알아서 다른 함수로 들어감. 

// 사용 이유: 실무에선 함수 내부를 안 보고 접어두기 때문.
// 이름만 보아도 뭔지 구분할 수 있게 하려고. 


//**함수 템플릿
// 사용방법:  T = 자료형  
// 인자의 자료형 다르고, 내부 코드 다 똑같을때. 
...
// 템플릿 함수로 넘겨줄때. 
// 꺽쇠 안에 자료형을 넣어서 선언한다. 
// 그러면 위 템플릿 함수에서 T가 전부 <>안에 적힌 자료형으로 치환된다. 
result = add <int>(4, 6);
result1 = add <char>('4', '6');
